...
function ChannelCard({ channel, onClick }) {
  const title = channel?.title || channel?.name || channel?.channel_name || "Canal sin nombre";
  const poster = channel?.poster_url || channel?.poster || channel?.thumbnail || "/poster-fallback.jpg";
  ...
  return (
    <button
      ...
      style={{ contentVisibility: "auto", containIntrinsicSize: "320px 480px" }} // ⬅️ acelera scroll/render
    >
      <div className="relative">
        ...
        <div className="absolute left-2 top-2">
          <span className="text-[11px] px-2 py-1 rounded-full bg-rose-500/90 text-white shadow">{category}</span>
        </div>
      </div>
      ...
      <div className="p-3 space-y-1.5">
        ...
        {description ? (
          <p className="text-[12px] leading-4 text-gray-400 line-clamp-2 mt-1.5">{description}</p>
        ) : null}
      </div>
    </button>
  );
